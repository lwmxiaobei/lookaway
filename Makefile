...
## 打包成 build/lookaway.app（带 Info.plist 与签名）
app:
	./scripts/bundle.sh
...
stop:
	@-pkill -f "lookaway.app/Contents/MacOS/LookAway" 2>/dev/null || true
	@sleep 1
...
selftest: app stop
	open build/lookaway.app --env LOOKAWAY_SELFTEST=1

## 诊断多屏：每块屏涂不同颜色并标注编号，遮 15 秒
debug-overlay: app stop
	open build/lookaway.app --env LOOKAWAY_DEBUG_OVERLAY=1 --env LOOKAWAY_SELFTEST=1 --env LOOKAWAY_SELFTEST_SECONDS=15

## 记录姿态轨迹到 /tmp/lookaway-trace.csv，用来诊断零点和误触发
trace: app stop
	open build/lookaway.app --env LOOKAWAY_TRACE=1
